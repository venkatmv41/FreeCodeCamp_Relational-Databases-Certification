...
echo "$($PSQL "SELECT AVG(winner_goals + opponent_goals) AS total_goals FROM games;  ")"
...
echo "$($PSQL "SELECT teams.name FROM teams INNER JOIN games ON teams.team_id=games.winner_id OR teams.team_id = games.opponent_iD WHERE year = 2014 and round = 'Eighth-Final' ORDER BY teams.name ")"
